...
public:
  ...
  void insert(size_t        value,
    ...
    for(int n=31;n>=0;n--) {
      ...
      if(current_bit) current_node = dtreestore.at(current_node).node_1;
                 else current_node = dtreestore.at(current_node).node_0;
    ...
    BinaryNode b_current_node = dtreestore.at(current_node);

    b_current_node.set_uri(uri);
    b_current_node.start_index = value;
    b_current_node.length      = length;

    dtreestore.at(current_node) = b_current_node;
  }
  ...
  void find_bounding(size_t index,
    ...
    BinaryNode b_current_node = dtreestore.at(current_node);
    for(;;) {
      b_current_node = dtreestore.at(current_node);
